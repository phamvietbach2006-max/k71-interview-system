...
start_marker = "Quản Lý Nhân Sự</h2>"
start_idx = code.find(start_marker)
if start_idx != -1:
    start_idx = code.rfind("<h2", 0, start_idx + len(start_marker))
    
    table_end_idx = code.find("</table>", start_idx)
    div_end_idx = code.find("</div>", table_end_idx) + 6
    
    code = code[:start_idx] + new_ui + code[div_end_idx:]
