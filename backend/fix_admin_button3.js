...
const matchIndex = code.indexOf(`{['Tr n ?cc HoAng Anh', 'Ki?u Minh Anh', 'Phm Vit BAch'].includes(user.fullName) && (`);
if (matchIndex !== -1) {
  const endMatch = code.indexOf(`</>`, matchIndex);
  if (endMatch !== -1) {
    const endBrace = code.indexOf(`)}`, endMatch);
    if (endBrace !== -1) {
       const toReplace = code.substring(matchIndex, endBrace + 2);
